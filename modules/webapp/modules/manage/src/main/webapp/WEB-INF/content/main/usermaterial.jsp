...
<rapid:override name="content">
  <div class="demo_rtdbq02 out_form_div" id="mytest">
	<form onSubmit="resetValue();" id="queryForm" name="queryForm" action="${ctx}/main/user-material.action" method="post" style="display: inline;">
	  	<!--最上行区域  -->
	  	<div class="tf_main_bt">
			...
			<div class="tf_main_bt_rt" >
			    <input type="hidden" id="owner" value="<s:property value='userMaterialQuery.owner'/>" name="userMaterialQuery.owner" />
			    ...
			    <div class="tf_inquire_nr">
			        ...
			        <table width="100%" border="0" cellspacing="0" cellpadding="0" style="line-height:35px;">
			            <tr>
			                ...
			                <td style="text-align:left;"><input  type="text" id="name" name="userMaterialQuery.name" value="<s:property value='userMaterialQuery.name'/>" ></td>
			                <td><span class="wz">类型</span></td>
			                <td style="text-align:left;"><span class="select_pre_span">
			                                             <select id="matelType" name="userMaterialQuery.matelType">
			                                                 <option value="-1">请选择</option>
			                                                 <s:iterator value="@com.manage.model.main.MaterialExt$MaType@values()" >
			                                                     <option value="${value}" <s:if test="value == userMaterialQuery.matelType">selected="selected"</s:if> >${name}</option>
			                                                 </s:iterator>
			                   							 </select>
			                   							 </span> 
			                </td>
			                <td><span class="wz">创建时间</span></td>
			                <td style="text-align:left;"><input type="text" id="createTimeFrom" name="userMaterialQuery.createTimeFrom" 
			                    value="<s:property value='userMaterialQuery.createTimeFromStr'/>"  style=" width:81px;" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd'})">
			                    <span class="wz" style="text-align:center;width:25px;padding-left:3px;">至</span>
			                    <input type="text" id="createTimeTo" name="userMaterialQuery.createTimeTo" style=" width:81px;" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd'})" 
			                    value="<s:property value='userMaterialQuery.createTimeToStr'/>" >
			                </td>
			            </tr>
			            <tr>
			...
			<tr class="tf_table_th">
			 ...
			 <c:forEach items="${page.result}" var="item" varStatus="status" >
			  ...
			  <tr class="demo_table02_lb ${status.count % 2 == 0 ? 'demo_table02_lb02' : ''}">
				...
				<td><c:out value='${item.material.maTypeStr}'/>&nbsp;</td>
				...
				<td><c:out value='${item.playerName}'/>&nbsp;</td>
				<td><c:out value='${item.count}'/>&nbsp;</td>
				
				<td><c:if test="${item.matelType==0}">
				    <c:choose><c:when test="${item.state==0}">未使用</c:when><c:otherwise>使用</c:otherwise></c:choose>
				    </c:if>
				    <c:if test="${item.matelType==2}">
				    <c:choose><c:when test="${item.place==0}">已装备</c:when><c:when test="${item.place>=1}">仓库中</c:when><c:otherwise>回收中</c:otherwise></c:choose>
				    </c:if>
				</td>
				
				<td><c:out value='${item.createTimeStr}'/>&nbsp;</td>
