...
<rapid:override name="head">
	...
	<script type="text/javascript" >
		...
		function showAddDialog()
		{
		    ...
		    $("#hiddencontent").find("h2").html("模块新增");
		    var pname=window.parent.frames["left"].reAsNode.name;
		    $("#parentName").val(pname);
		    
		    $("#hiddenout").show();
		    $("#hiddencontent").show();
		    $("#inputForm").attr("action","${ctx}/rbac/module!save.action");
		}
		//关闭新增/修改面板
		...
		{
		    var htmlObj;
		    htmlObj=$.ajax({url:"${ctx}/rbac/module!info.action",async:false,type:"POST",data:{"id":id}
		    });
		    var moduleResult=htmlObj.responseText;
		    moduleResult=jQuery.parseJSON(moduleResult);
		    $("#id").val(id);
		    $("#name").val(moduleResult.name);

		    if(!isBlank(moduleResult.url) )
		    {
		        $("#url").val(moduleResult.url);
		    }
		    if(!isBlank(moduleResult.parentName) )
		    {
		        $("#parentName").val(moduleResult.parentName);
		    }else
		    {
		        var pname=window.parent.frames["left"].reAsNode.name;
		        $("#parentName").val(pname);
		    }
		    if(moduleResult.sort!=null && isNumber(moduleResult.sort) )
		    {
		         $("#sort").val(moduleResult.sort);
		    }
		    $("#hiddencontent").find("h2").html("模块修改");
		    $("#hiddenout").show();
		    $("#hiddencontent").show();
		    $("#inputForm").attr("action","${ctx}/rbac/module!save.action");
		}
		...
		function submitDialog()
		{
           ...
           if( !isBlank($("#sort").val()) && !isNumber($("#sort").val()) )
           {
                alert("排序必须是数字!");
                return;
           }		   
            ...
            var options={
	            ...
	            {    
	                ...
	                if(result)
	                {
	                    alert("操作成功!");
	                    closeDialog();
	                    window.parent.frames["left"].reAsyn();
	                    $("#refresh").val(1);
	                    $("#queryForm").submit();
	                }else
	                {
		...
		function showInfoDialog(id)
		{
		    ...
		    $("#name2").val(moduleResult.name);
		    if(!isBlank(moduleResult.url) )
		    {
		        $("#url2").val(moduleResult.url);
		    }
		    if(!isBlank(moduleResult.parentName) )
		    {
		        $("#parentName2").val(moduleResult.parentName);
		    }else
		    {
		        var pname=window.parent.frames["left"].reAsNode.name;
		        $("#parentName2").val(pname);
		    
		    }
		    if(moduleResult.sort!=null && isNumber(moduleResult.sort) )
		    {
		         $("#sort2").val(moduleResult.sort);
		    }
		    
		    $("#hiddencontent2").find("h2").html("模块信息");
		    $("#hiddenout").show();
		    $("#hiddencontent2").show();
		}
		//关闭详情面板
		...
		function del()
		{
		    ...
		    var options={
	            url:"${ctx}/rbac/module!delete.action",
	            type:"POST",
	            dataType:"script",
	            success:function(msg)
	            {    
				    ...
				    {
				       alert("选中记录中有子模块,不能直接删除!");
				       
				    }else if(result==3)
				    {
				       alert("选中记录中挂有资源,不能直接删除!");
...
<rapid:override name="content">
  <div class="demo_rtdbq02 out_form_div" id="mytest">
	<form onSubmit="resetValue();" id="queryForm" name="queryForm" action="${ctx}/rbac/module!hierarchyPage.action" method="post" >
	    ...
	    <!--最上行区域  -->
	  	<div class="tf_main_bt">
			...
			<div class="tf_main_bt_lf">
			    <%
			        boolean canCreate=AdminorHolder.getInstance().hasPermission("com.manage.rbac.module",OperationFlag.CREATE);
			        if(canCreate) {
			    %>
				<a  href="javascript:;" onclick="showAddDialog('${item.id}');return false;"><span>新&nbsp;增</span></a>
				<% } %>
				<%
			        boolean canDelete=AdminorHolder.getInstance().hasPermission("com.manage.rbac.module",OperationFlag.DELETE);
			        if(canDelete) {
			    %>
			...
			<tr class="tf_table_th">
			 ...
			 <c:forEach items="${page.result}" var="item" varStatus="status">
			  ...
			  <tr class="demo_table02_lb ${status.count % 2 == 0 ? 'demo_table02_lb02' : ''}">
				...
				<td>
				    <%
				        boolean canUpdate=AdminorHolder.getInstance().hasPermission("com.manage.rbac.module",OperationFlag.UPDATE);
			            if(canUpdate) {
			        %>
...
<div id="hiddencontent">
	...
	<input type="hidden" id="pid" name="pid" value="${parentid}"/>
	<input type="hidden" id="id" name="id" />
	   <div class="tablediv">
	      <table cellpadding="0" cellspacing="2" border="0" class="hiddentable" >
	          ...
	          <tr>  
	            <td class="td_title">父模块名</td>
	            <td class="td_input">
	                <input id="parentName" name="parentName" type="text" readonly="readonly" disabled="disabled" class="fil"/> 
	            </td>
	          </tr>
	          <tr>  
	            <td class="td_title">访问路径</td>
	            <td class="td_input">
	                <input id="url" name="url" type="text" maxlength="200" class="type:string fil"/> 
	            </td>
	          </tr>
	          <tr>  
	            <td class="td_title">排&nbsp;&nbsp;序</td>
	            <td class="td_input">
	                <input id="sort" name="sort" type="text" maxlength="3" class="type:int fil" /> 
	            </td>
	          </tr>
	          <tr>
	   ...
	   <div class="tablediv">
	      <table cellpadding="0" cellspacing="2" border="0" class="hiddentable" >
	          ...
	          <tr>  
	            <td class="td_title">父模块名</td>
	            <td class="td_input">
	                <input id="parentName2" name="parentName2" type="text" readonly="readonly" disabled="disabled" class="fil"/> 
	            </td>
	          </tr>
	          <tr>  
	            <td class="td_title">访问路径</td>
	            <td class="td_input">
	                <input id="url2" name="url2" type="text" maxlength="200" class="fil" readonly="readonly" disabled="disabled" /> 
	            </td>
	          </tr>
	          <tr>  
	            <td class="td_title">排&nbsp;&nbsp;序</td>
	            <td class="td_input">
	                <input id="sort2" name="sort2" type="text" maxlength="3" class="fil" readonly="readonly" disabled="disabled" /> 
	            </td>
	          </tr>
	          <tr>
	             <td colspan="2" style="text-align:center">
