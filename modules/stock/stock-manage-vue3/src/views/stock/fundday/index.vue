<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          icon="Download"
          @click="handleExport"
          v-hasPermi="['stock:fundday:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar v-model:showSearch="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="funddayList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['stock:fundday:edit']">修改</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="funddayRef" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="基金名称" prop="fundName">
...
<script setup name="Fundday">
import { listFundday, getFundday, delFundday, addFundday, updateFundday } from "@/api/stock/fundday";

const { proxy } = getCurrentInstance();

const funddayList = ref([]);
...
function getList() {
  loading.value = true;
  listFundday(queryParams.value).then(response => {
    funddayList.value = response.rows;
...
/** 提交按钮 */
function submitForm() {
  proxy.$refs["funddayRef"].validate(valid => {
    if (valid) {
      if (form.value.id != null) {
        updateFundday(form.value).then(response => {
          proxy.$modal.msgSuccess("修改成功");
          open.value = false;
          getList();
        });
      } else {
        addFundday(form.value).then(response => {
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除基金天数据编号为"' + _ids + '"的数据项？').then(function() {
    return delFundday(_ids);
  }).then(() => {
    getList();
...
function handleExport() {
  proxy.download('stock/fundday/export', {
    ...queryParams.value
  }, `fundday_${new Date().getTime()}.xlsx`)
}
