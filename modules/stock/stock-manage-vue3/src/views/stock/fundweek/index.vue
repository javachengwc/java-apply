<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          icon="Download"
          @click="handleExport"
          v-hasPermi="['stock:fundweek:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="fundweekRef" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="基金名称" prop="fundName">
...
<script setup name="Fundweek">
import { listFundweek, getFundweek, delFundweek, addFundweek, updateFundweek } from "@/api/stock/fundweek";

const { proxy } = getCurrentInstance();

const fundweekList = ref([]);
...
function getList() {
  loading.value = true;
  listFundweek(queryParams.value).then(response => {
    fundweekList.value = response.rows;
...
/** 提交按钮 */
function submitForm() {
  proxy.$refs["fundweekRef"].validate(valid => {
    if (valid) {
      if (form.value.id != null) {
        updateFundweek(form.value).then(response => {
          proxy.$modal.msgSuccess("修改成功");
          open.value = false;
          getList();
        });
      } else {
        addFundweek(form.value).then(response => {
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除基金周数据编号为"' + _ids + '"的数据项？').then(function() {
    return delFundweek(_ids);
  }).then(() => {
    getList();
...
function handleExport() {
  proxy.download('stock/fundweek/export', {
    ...queryParams.value
  }, `fundweek_${new Date().getTime()}.xlsx`)
}
