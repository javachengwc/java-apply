...
export function isFilePath(path) {
  return /^\//.test(path) && /\..{1,5}/.test(path);
}
 ...
 * getPopupContainer
 */
export const getContianer = () => {
  return document.querySelector('.ant-layout-content');
};

export const convertObj2Params = obj => {
...
export const requestIsSuccess = res => get(res, 'header.ret') === 'S';
...
export function getUrl(params, downUrl) {
  // 拼接url地址\
  let url = downUrl;
