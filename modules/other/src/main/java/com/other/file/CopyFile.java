...
public class CopyFile extends JFrame{
		...
		copyButton.addActionListener(new ActionListener(){	//拷贝按钮事件处理
			public void actionPerformed(ActionEvent event) {
				...
				if (copy(sourceFile,targetFile)){	//拷贝文件
					JOptionPane.showMessageDialog(CopyFile.this,"拷贝成功");	//显示拷贝成功信息
				}
				else{
					JOptionPane.showMessageDialog(CopyFile.this,"拷贝失败");	//发生错误
				}
			}
		});
	...
	class SelectFileListener implements ActionListener {	//取得目录内容的事件处理
		public void actionPerformed(ActionEvent event) {
			if (fileChooser.showOpenDialog(CopyFile.this)==JFileChooser.APPROVE_OPTION){	//弹出文件选择器,并判断是否点击了打开按钮
